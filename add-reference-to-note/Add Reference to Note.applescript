...
property configFile : «event sysoload» (POSIX path of («event earsffdr» «constant afdrcusr») & ".workflowscripts/config.scpt")
...
on writeLog(theMessage)
	set timestamp to «event sysoexec» "date \"+%Y-%m-%d %H:%M:%S\""
	«event sysoexec» "printf '%s\n' " & quoted form of (timestamp & ": " & pScriptName & ": " & theMessage) & " >> " & quoted form of pLogFile
end writeLog
